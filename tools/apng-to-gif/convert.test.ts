/**
 * S1 spike (T1.3, PLAN.md 前置 spikes): gifenc node PoC, plus (T2.2) the full
 * byte-level test suite for `convert.ts`'s `convertToGif`.
 *
 * The first `describe` block below predates `convert.ts` and exercises
 * `gifenc` directly with hand-built RGBA frames and byte-level readback (via
 * ./gif-reader.ts) to lock in facts `convert.ts` depends on: GIF89a
 * header/trailer/frame count, the delay ms->cs contract, the finite-loop
 * mapping, and transparent flag/index round-tripping. It's kept as a
 * library-contract regression guard independent of our own code.
 *
 * The later blocks test `convert.ts` itself (see PLAN.md「Verification」).
 */
...
function makeRgbaFrame(width: number, height: number, pixels: ReadonlyArray<readonly [number, number, number, number]>): RGBAFrame {
...
function makeSolidFrame(width: number, height: number, r: number, g: number, b: number, a = 255): RGBAFrame {
  const pixelCount = width * height
  return makeRgbaFrame(width, height, Array.from({ length: pixelCount }, () => [r, g, b, a] as const))
}

describe('normalizeConvertOptions (T2.2)', () => {
  ...
  it('clamps out-of-range scalar options into their valid ranges', () => {
    expect(normalizeConvertOptions({ alphaThreshold: -50 }).alphaThreshold).toBe(0)
    expect(normalizeConvertOptions({ alphaThreshold: 999 }).alphaThreshold).toBe(255)
    expect(normalizeConvertOptions({ maxColors: 0 }).maxColors).toBe(2)
    expect(normalizeConvertOptions({ maxColors: 5000 }).maxColors).toBe(256)
  })

  it('rounds fractional scalar options', () => {
    expect(normalizeConvertOptions({ alphaThreshold: 127.6 }).alphaThreshold).toBe(128)
  })

  it('falls back to the default matte color when any channel is invalid (NaN)', () => {
    expect(normalizeConvertOptions({ matteColor: [Number.NaN, 10, 10] }).matteColor).toEqual(DEFAULT_MATTE_COLOR)
  })

  it('clamps a valid but out-of-range matte color channel-wise', () => {
    expect(normalizeConvertOptions({ matteColor: [-10, 300, 128] }).matteColor).toEqual([0, 255, 128])
  })
})
...
describe('convertToGif (T2.2)', () => {
  it('rejects an animation with no frames', () => {
    const anim: DecodedAnimation = { frames: [], delaysMs: [], loop: 0 }
    expect(() => convertToGif(anim)).toThrow()
  })
  ...
  it("writes each frame's own delay in centiseconds (ms/10 contract) for distinct values", () => {
    ...
    const gces = parseGif(convertToGif(anim)).graphicControlExtensions
  ...
  it('clamps a below-floor delay to the 20ms floor (2cs) end-to-end', () => {
    ...
    const gce = parseGif(convertToGif(anim)).graphicControlExtensions[0]
  ...
  describe('loop mapping end-to-end', () => {
    it.each([
      ...
      const anim: DecodedAnimation = { frames: [makeSolidFrame(2, 2, 1, 2, 3)], delaysMs: [100], loop }
      expect(parseGif(convertToGif(anim)).loopCount).toBe(expectedLoopCount)
    })
  })
  ...
  it('normalizes invalid options instead of throwing', () => {
    const anim: DecodedAnimation = { frames: [makeSolidFrame(2, 2, 1, 2, 3)], delaysMs: [100], loop: 0 }
    expect(() =>
      convertToGif(anim, { alphaThreshold: Number.NaN, matteColor: [Number.NaN, Number.NaN, Number.NaN], maxColors: -5 }),
    ).not.toThrow()
  })
  ...
  it('calls onFrameEncoded once per frame with a strictly increasing 0-based index and a constant total (T3.3)', () => {
    ...
    convertToGif(anim, { onFrameEncoded })
